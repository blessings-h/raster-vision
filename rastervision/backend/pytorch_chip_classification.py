...
class PyTorchChipClassification(Backend):
    ...
    def train(self, tmp_dir):
        ...
        databunch = build_databunch(
            chip_dir, chip_size, batch_size, self.task_config.get_class_names())
        num_labels = len(databunch.label_names)
        if self.train_opts.debug:
            make_debug_chips(
                databunch, self.task_config.class_map, tmp_dir, train_uri)
...
=======
                self.train_opts.model_arch, num_classes, pretrained=True,
                progress=True)
>>>>>>> b787f7a... Convert PyTorch chip classification to pure PyTorch
            model = model.to(self.device)
            model.load_state_dict(
                torch.load(model_path, map_location=self.device))
            self.model = model
